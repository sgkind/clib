...
void
svec_clone(svec_t *svec, const svec_t *other)
{
    svec_init(svec);
...
bool
svec_is_empty(const svec_t *svec)
{
    return svec->n == 0;
}

void
svec_add(svec_t *svec, const char *name)
{
    svec_add_nocopy(svec, strdup(name));
}

void
svec_del(svec_t *svec, const char *name)
{
    size_t offset;
...
void
svec_add_nocopy(svec_t *svec, char *name)
{
    svec_expand(svec);
...
void
svec_append(svec_t *svec, const svec_t *other)
{
    size_t i;
    for (i = 0; i < other->n; i++) {
...
void
svec_diff(const svec_t *a, const svec_t *b,
...
bool
svec_contains(const svec_t *svec, const char *name)
{
    return svec_find(svec, name) != SIZE_MAX;
}

size_t
svec_find(const svec_t *svec, const char *name)
{
    char **p;
...
bool
svec_is_sorted(const svec_t *svec)
{
    size_t i;
...
bool
svec_is_unique(const svec_t *svec)
{
    return svec_get_duplicate(svec) == NULL;
}

const char *
svec_get_duplicate(const svec_t *svec)
{
    assert(svec_is_sorted(svec));
    if (svec->n > 1) {
        size_t i;
...
void
svec_print(const svec_t *svec, const char *title)
{
    size_t i;
...
/*
void
svec_parse_words(svec_t *svec, const char *words)
{
...
*/
bool
svec_equal(const svec_t *a, const svec_t *b)
{
    size_t i;
